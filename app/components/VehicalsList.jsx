...
import { ArrowUpRight, Gauge, Fuel, Settings, Heart, ChevronDown, ChevronUp } from "lucide-react" // Replaced react-icons with lucide-react
...
const VehicalsList = ({ loadingState }) => {
  ...
  const fetchVehicles = async () => {
    ...
    // Check cache first
    if (apiCache[cacheKey] && Date.now() - apiCache[cacheKey].timestamp < 180000) {
      // 3 min cache
  ...
  const handleToggleVisibility = () => {
    if (visibleVehiclesCount >= vehicles.length) {
      ...
      setVisibleVehiclesCount((prevCount) => Math.min(prevCount + 3, vehicles.length))
    }
  }
    ...
    <section className="my-7 rounded-xl bg-slate-50 py-7 dark:bg-slate-900 sm:mx-8 md:my-10 md:py-10">
                      ...
                      <div className="absolute bottom-4 right-4 rounded-2xl bg-white/95 px-4 py-2 shadow-lg backdrop-blur-md dark:bg-slate-800/95">
                        <div className="text-right">
                          ...
                          <p className="bg-gradient-to-r from-slate-900 to-slate-600 bg-clip-text text-lg font-bold text-transparent dark:from-white dark:to-slate-300">
                            ...
                            {Math.round(
                              (vehicle?.price * (selectedCurrency?.value || 1)) / (currency?.value || 1),
                            ).toLocaleString()}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="mb-6 space-y-3">
                      <div className="flex items-center gap-3 text-sm">
                        ...
                        <span className="font-semibold text-slate-900 dark:text-white">
                          {convertedValues.kms} {convertedValues.unit?.toUpperCase()}
                        </span>
                      </div>
                      <div className="flex items-center gap-3 text-sm">
                    ...
                    <Link href={`/car-detail/${vehicle.slug || vehicle._id}`} className="group/cta block w-full">
                      <div className="transform rounded-2xl bg-gradient-to-r from-slate-900 to-slate-700 px-6 py-3.5 text-center font-semibold text-white shadow-lg transition-all duration-300 md:hover:scale-[1.02] hover:from-slate-800 hover:to-slate-600 hover:shadow-xl dark:from-slate-100 dark:to-slate-300 dark:text-slate-900 dark:hover:from-white dark:hover:to-slate-200">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <ArrowUpRight className="h-4 w-4 transition-transform duration-300 md:group-hover/cta:-translate-y-1 md:group-hover/cta:translate-x-1" />
                        </div>
                      </div>
                    </Link>
                  </div>
      ...
      {!loading && vehicles.length > 3 && (
        <div className="mt-10 text-center">
          ...
          >
            <span>{visibleVehiclesCount >= vehicles.length ? "Show less" : "Show more"}</span>
      ...
      {vehicles.length === 0 && !loading && (
        <div className="py-20 text-center">
          ...
          <h3 className="mb-4 text-2xl font-bold text-slate-900 dark:text-white">No Vehicles Available</h3>
