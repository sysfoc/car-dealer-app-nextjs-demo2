...
const sanitizeCookieConsent = (settings: CookieboxProps["cookieConsent"]): NonNullable<CookieboxProps["cookieConsent"]> => {
  ...
  const sanitized = {
    ...
    buttonTextColor: isValidHexColor(settings.buttonTextColor) ? settings.buttonTextColor : defaultSettings.buttonTextColor,
...
const Cookiebox = ({ cookieConsent: propsCookieConsent }: CookieboxProps) => {
  ...
  // Fetch settings with retry logic and error handling
  const fetchSettings = async (attempt: number = 0) => {
    const maxRetries = 3;
  ...
  useEffect(() => {
    if (!isLoading && cookieConsent) {
      try {
        ...
        // If storage check fails, default to showing the cookie box if consent is active
        if (cookieConsent.status === 'active') {
          setIsVisible(true);
        }
      }
    }
    ...
    try {
      ...
      if (typeof window !== 'undefined' && typeof window.gtag === 'function') {
        ...
        try {
          ...
          console.error('Error updating Google Analytics consent:', gtagError);
        }
      } else {
        console.log('Google Analytics not available or gtag function not found');
      }
