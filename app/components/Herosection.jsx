...
const HeroSection = ({ initialHeading }) => {
  ...
  // Process heading (no state needed since data comes from server)
  const processedHeading = useMemo(() => {
    if (!initialHeading) return null;
    
    const words = initialHeading.split(" ");
    if (words.length <= 2) return initialHeading;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (carImages[1]) {
        const existingLink = document.head.querySelector(`link[href="${carImages[1]}"]`);
    ...
    <section className="relative h-screen w-full overflow-hidden">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8">
        <div className="w-full max-w-6xl space-y-8 text-center">
          ...
          {/* Main Heading - No loading state needed, data comes from server */}
          <div className="space-y-6">
            <h1 className="text-center text-4xl font-bold leading-tight text-white drop-shadow-2xl sm:text-5xl lg:text-6xl xl:text-7xl">
              {processedHeading ? (
                typeof processedHeading === 'string' ? (
                  processedHeading
                ) : (
                  <>
